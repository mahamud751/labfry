...
export class AuthService {
  ...
  async register(data: RegisterDto): Promise<AuthResponse> {
    try {
      ...
      const emailVerificationToken =
        this.jwtService.generateEmailVerificationToken(data.email);
  ...
  async login(data: LoginDto): Promise<AuthResponse> {
    try {
      ...
      const sessionExpireTime =
        this.configService.get("SESSION_EXPIRE_TIME") || 24 * 60 * 60 * 1000;
      const refreshExpireTime =
        this.configService.get("REFRESH_SESSION_EXPIRE_TIME") ||
        7 * 24 * 60 * 60 * 1000;
      ...
      const finalRefreshToken =
        this.jwtService.generateRefreshToken(updatedJwtPayload);
  ...
  async verifyEmail(data: VerifyEmailDto): Promise<AuthResponse> {
    try {
      ...
      if (data.token) {
        const { email } = this.jwtService.verifyEmailVerificationToken(
          data.token
        );
  ...
  async refreshToken(refreshToken: string): Promise<AuthResponse> {
    try {
      ...
      const newRefreshToken =
        this.jwtService.generateRefreshToken(newJwtPayload);
  ...
  async logout(sessionId: string): Promise<AuthResponse> {
    try {
      ...
      if (session) {
        // Session exists, mark it as inactive
        await this.prisma.session.update({
          where: { id: sessionId },
          data: { isActive: false },
        });
      }
      // If session doesn't exist, we still consider logout successful
      ...
      // Return success even if there's an error - logout should always succeed from user perspective
      return {
        success: true,
        message: "Logged out successfully",
      };
  ...
  async resendVerificationCode(email: string): Promise<AuthResponse> {
    try {
      ...
      const emailVerificationCode = generateVerificationCode();
      const emailVerificationToken =
        this.jwtService.generateEmailVerificationToken(email);
